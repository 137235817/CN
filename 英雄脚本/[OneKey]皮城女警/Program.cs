...
namespace Caitlyn
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            SpellList.Add(R1);
            //Create the menu
            Config = new Menu("[OneKey]皮城女警", ChampionName, true);
            ...
            Config.AddItem(new MenuItem("autoR", "自动R").SetValue(true));
            Config.AddItem(new MenuItem("Hit", "Q命中率").SetValue(new Slider(2, 2, 0)));
            Config.AddItem(new MenuItem("useR", "半自动大招按键").SetValue(new KeyBind('t', KeyBindType.Press))); //32 == space
            #region E
                Config.SubMenu("E Config").AddItem(new MenuItem("autoE", "自动E").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("opsE", "使用W").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("AGC", "自动E防突进").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("useE", "一键释放E").SetValue(new KeyBind('t', KeyBindType.Press)));
            #endregion
            ...
            Game.PrintChat("<font color=\"#7e62cc\">C</font>aitlyn full automatic SI ver 1.5 <font color=\"#000000\">by sebastiank1</font> - <font color=\"#00BFFF\">鍔犺浇鎴愬姛  Vee姹夊寲</font>");
        }
