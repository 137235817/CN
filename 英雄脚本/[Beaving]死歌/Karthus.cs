    ...
    * */
    ...
    internal class Karthus
    {
        ...
        public Karthus()
        {
            ...
            (_menu = new Menu("死歌", "KarthusSharp", true)).AddToMainMenu();
            ...
            comboMenu.AddItem(new MenuItem("comboAA", "使用平A").SetValue(false));
            comboMenu.AddItem(new MenuItem("comboWPercent", "W控蓝%").SetValue(new Slider(10)));
            comboMenu.AddItem(new MenuItem("comboEPercent", "E控蓝%").SetValue(new Slider(15)));
            comboMenu.AddItem(new MenuItem("comboMove", "走砍/移动").SetValue(true));

            var harassMenu = _menu.AddSubMenu(new Menu("骚扰", "Harass"));
            harassMenu.AddItem(new MenuItem("harassQ", "使用Q").SetValue(true));
            harassMenu.AddItem(new MenuItem("harassQPercent", "Q控蓝%").SetValue(new Slider(15)));
            harassMenu.AddItem(new MenuItem("harassQLasthit", "优先补刀").SetValue(true));
            harassMenu.AddItem(new MenuItem("harassMove", "走砍/移动").SetValue(true));

            var farmMenu = _menu.AddSubMenu(new Menu("补兵", "Farming"));
            farmMenu.AddItem(new MenuItem("farmQ", "使用Q").SetValue(new StringList(new[] { "尾刀", "清线", "都用", "不用" }, 1)));
            farmMenu.AddItem(new MenuItem("farmE", "清线使用E").SetValue(true));
            farmMenu.AddItem(new MenuItem("farmAA", "清线使用平A").SetValue(false));
            farmMenu.AddItem(new MenuItem("farmQPercent", "Q控蓝%").SetValue(new Slider(10)));
            farmMenu.AddItem(new MenuItem("farmEPercent", "E控蓝%").SetValue(new Slider(20)));
            farmMenu.AddItem(new MenuItem("farmMove", "走砍/移动").SetValue(true));

            var notifyMenu = _menu.AddSubMenu(new Menu("大招可击杀时提醒", "Notify"));
            notifyMenu.AddItem(new MenuItem("notifyR", "文字提醒").SetValue(true));
            notifyMenu.AddItem(new MenuItem("notifyPing", "Ping提醒").SetValue(false));

            var drawMenu = _menu.AddSubMenu(new Menu("显示设置", "Drawing"));
            drawMenu.AddItem(new MenuItem("drawQ", "显示Q范围").SetValue(new Circle(true, System.Drawing.Color.FromArgb(125, 0, 255, 0))));

            var miscMenu = _menu.AddSubMenu(new Menu("其他", "Misc"));
            miscMenu.AddItem(new MenuItem("ultKS", "大招击杀").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoCast", "死亡后自动连招/清线").SetValue(false));
            miscMenu.AddItem(new MenuItem("packetCast", "使用封包").SetValue(true));
            ...
            Game.PrintChat("<font color=\"#1eff00\">KarthusSharp by Beaving</font> - <font color=\"#00BFFF\">鍔犺浇鎴愬姛  Vee姹夊寲</font>");
        }
