...
class SkinManager
{
    ...
    public void AddToMenu(ref Menu menu)
    {
        ...
        if (Skins.Count > 0)
        {
            ...
            Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_enabled", "启用换肤").SetValue(false));
            Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_select", "选择皮肤").SetValue(new StringList(Skins.ToArray())));
