...
namespace EasyJinx
{
    class EasyKalista : Champion
    {
        ...
        protected override void InitializeMenu()
        {
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("Combo_erange", "敌人离开范围使用E").SetValue(true));
            Menu.SubMenu("Combo").AddItem(new MenuItem("Combo_e", "堆叠后使用E").SetValue(false));
            Menu.SubMenu("Combo").AddItem(new MenuItem("Combo_estacks", "堆叠层数").SetValue(new Slider(4, 1, 10)));

            Menu.AddSubMenu(new Menu("消耗", "Harass"));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_q", "使用Q").SetValue(true));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_e", "堆叠后使用E").SetValue(true));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_estacks", "堆叠层数").SetValue(new Slider(4, 1, 10)));
            
            Menu.AddSubMenu(new Menu("自动", "Auto"));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_q", "使用Q").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_ekill", "使用E击杀").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_e", "堆叠后使用E").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_estacks", "堆叠层数").SetValue(new Slider(4, 1, 10)));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_ejungle", "清野使用E").SetValue(true));
            
            Menu.AddSubMenu(new Menu("范围显示", "Drawing"));
            ...
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_r", "R范围").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_edamage", "E伤害指示器").SetValue(true));
