...
namespace FuckingAwesomeLeeSin
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Base menu
            Menu = new Menu("[FA]盲僧", ChampName, true);
            //Orbwalker and menu
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("useQ", "使用1段Q").SetValue(true));
            Menu.SubMenu("Combo").AddItem(new MenuItem("useQ2", "使用2段Q").SetValue(true));
            Menu.SubMenu("Combo").AddItem(new MenuItem("useW", "连招使用跳眼").SetValue(false));
            Menu.SubMenu("Combo").AddItem(new MenuItem("dsjk", "跳眼|如果范围: "));
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("useR", "使用R").SetValue(false));
            Menu.SubMenu("Combo").AddItem(new MenuItem("ksR", "R击杀").SetValue(false));
            ...
            harassMenu.AddItem(new MenuItem("q1H", "使用1段Q").SetValue(true));
            harassMenu.AddItem(new MenuItem("q2H", "使用2段Q").SetValue(true));
            harassMenu.AddItem(new MenuItem("wH", "跳眼/小兵 逃跑").SetValue(true));
            harassMenu.AddItem(new MenuItem("eH", "使用1段E").SetValue(false));
            Menu.AddSubMenu(harassMenu);

            //Jung/Wave Clear
            var waveclearMenu = new Menu("清线|清野", "wjClear");
            waveclearMenu.AddItem(new MenuItem("sjasjsdsjs", "清线"));
            waveclearMenu.AddItem(new MenuItem("useQClear", "使用Q").SetValue(true));
            waveclearMenu.AddItem(new MenuItem("useEClear", "使用E").SetValue(true));
            waveclearMenu.AddItem(new MenuItem("sjasjjs", "清野"));
            waveclearMenu.AddItem(new MenuItem("jungActive", "清线/清野").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
            waveclearMenu.AddItem(new MenuItem("Qjng", "使用Q").SetValue(true));
            waveclearMenu.AddItem(new MenuItem("Wjng", "使用W").SetValue(true));
            waveclearMenu.AddItem(new MenuItem("Ejng", "使用E").SetValue(true));
            Menu.AddSubMenu(waveclearMenu);

            //InsecMenu
            var insecMenu = new Menu("回旋踢", "Insec");
            insecMenu.AddItem(new MenuItem("InsecEnabled", "使用").SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Press)));
            insecMenu.AddItem(new MenuItem("rnshsasdhjk", "回旋踢模式:"));
            insecMenu.AddItem(new MenuItem("insecMode", "左键点击 [on] 内置目标选择 [off]").SetValue(true));
            insecMenu.AddItem(new MenuItem("insecOrbwalk", "走砍").SetValue(true));
            insecMenu.AddItem(new MenuItem("flashInsec", "闪现回旋踢").SetValue(false));
            insecMenu.AddItem(new MenuItem("waitForQBuff", "等待Q冷却").SetValue(false));
            insecMenu.AddItem(new MenuItem("22222222222222", "(更快 off 更多伤害 on)"));
            insecMenu.AddItem(new MenuItem("clickInsec", "点击回旋踢").SetValue(true));
            var lM = insecMenu.AddSubMenu(new Menu("点击回旋踢说明", "clickInstruct"));
            lM.AddItem(new MenuItem("1223342334", "首要目标选择"));
            lM.AddItem(new MenuItem("122334233", "选择目标回旋踢"));
            insecMenu.AddItem(new MenuItem("insec2champs", "踢向队友").SetValue(true));
            insecMenu.AddItem(new MenuItem("bonusRangeA", "踢向队友范围").SetValue(new Slider(0, 0, 1000)));
            insecMenu.AddItem(new MenuItem("insec2tower", "踢向塔").SetValue(true));
            insecMenu.AddItem(new MenuItem("bonusRangeT", "踢向塔范围").SetValue(new Slider(0, 0, 1000)));
            insecMenu.AddItem(new MenuItem("insec2orig", "踢向原来位置").SetValue(true));
            insecMenu.AddItem(new MenuItem("22222222222", "--"));
            insecMenu.AddItem(new MenuItem("instaFlashInsec1", "手动释放R"));
            insecMenu.AddItem(new MenuItem("instaFlashInsec2", "闪现R"));
            ...
            autoSmiteSettings.AddItem(new MenuItem("normSmite", "惩戒").SetValue(true));
            autoSmiteSettings.AddItem(new MenuItem("drawSmite", "显示惩戒范围").SetValue(true));
            Menu.AddSubMenu(autoSmiteSettings);

            //SaveMe Menu
            var SaveMeMenu = new Menu("留存惩戒设置", "Smite Save Settings");
            ...
            SaveMeMenu.AddItem(new MenuItem("param1", "生命值低于%不使用惩戒")); // TBC
            ...
            //Wardjump menu
            var wardjumpMenu = new Menu("跳眼", "Wardjump");
            wardjumpMenu.AddItem(
                new MenuItem("wjump", "跳眼按键").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
            wardjumpMenu.AddItem(new MenuItem("m2m", "跳向鼠标方向").SetValue(true));
            wardjumpMenu.AddItem(new MenuItem("j2m", "跳向小兵").SetValue(true));
            wardjumpMenu.AddItem(new MenuItem("j2c", "跳向英雄").SetValue(true));
            Menu.AddSubMenu(wardjumpMenu);

            var drawMenu = new Menu("显示", "Drawing");
            drawMenu.AddItem(new MenuItem("DrawEnabled", "启用").SetValue(false));
            drawMenu.AddItem(new MenuItem("drawST", "惩戒文字提示").SetValue(true));
            drawMenu.AddItem(new MenuItem("drawOutLineST", "显示轮廓").SetValue(true));
            drawMenu.AddItem(new MenuItem("insecDraw", "显示回旋踢").SetValue(true));
            drawMenu.AddItem(new MenuItem("WJDraw", "显示跳眼").SetValue(true));
            drawMenu.AddItem(new MenuItem("drawQ", "显示Q范围").SetValue(true));
            drawMenu.AddItem(new MenuItem("drawW", "显示W范围").SetValue(true));
            drawMenu.AddItem(new MenuItem("drawE", "显示E范围").SetValue(true));
            drawMenu.AddItem(new MenuItem("drawR", "显示R范围").SetValue(true));
            Menu.AddSubMenu(drawMenu);

            var miscMenu = new Menu("其他", "Misc");
            miscMenu.AddItem(new MenuItem("NFE", "使用封包").SetValue(true));
            miscMenu.AddItem(new MenuItem("QHC", "Q命中率").SetValue(new StringList(new []{"低", "中", "高", "非常高"}, 1)));
            miscMenu.AddItem(new MenuItem("IGNks", "使用点燃?").SetValue(true));
            miscMenu.AddItem(new MenuItem("qSmite", "惩戒Q!").SetValue(true));
