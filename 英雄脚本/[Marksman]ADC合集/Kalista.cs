...
namespace Marksman
{
    ...
    internal class Kalista : Champion
    {
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseQTH" + Id, "使 Q(自动)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            return true;
        }
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawConnText" + Id, "绑定 提示").SetValue(new Circle(false, Color.GreenYellow)));
            config.AddItem(
                new MenuItem("DrawConnSignal" + Id, "绑定 信号").SetValue(true));
            config.AddItem(new MenuItem("Dx" + Id, ""));
            config.AddItem(
                new MenuItem("DrawEStackCount" + Id, "E堆叠层数").SetValue(new Circle(false, Color.Firebrick)));
            config.AddItem(new MenuItem("Dx", ""));
            config.AddItem(
                new MenuItem("DrawJumpPos" + Id, "跳跃点").SetValue(new Circle(false, Color.HotPink)));

            var damageAfterE = new MenuItem("DamageAfterE", "显示E伤害").SetValue(true);
