...
//        public static Menu MenuInterruptableSpell;
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            var summonersHeal = summoners.AddSubMenu(new Menu("治疗", "Heal"));
            {
                summonersHeal.AddItem(new MenuItem("SUMHEALENABLE", "启用").SetValue(true));
                summonersHeal.AddItem(new MenuItem("SUMHEALSLIDER", "生命值<= %时使用治疗.").SetValue(new Slider(20, 99, 1)));
            }
            ...
            var summonersBarrier = summoners.AddSubMenu(new Menu("屏障", "Barrier"));
            {
                ...
                summonersBarrier.AddItem(
                    new MenuItem("SUMBARRIERSLIDER", "生命值<= %时使用屏障.").SetValue(new Slider(20, 99, 1)));
            }

            var summonersIgnite = summoners.AddSubMenu(new Menu("引燃", "Ignite"));
            {
            ...
            items.AddItem(new MenuItem("GHOSTBLADE", "幽梦").SetValue(true));
            items.AddItem(new MenuItem("SWORD", "神圣之剑").SetValue(true));
            items.AddItem(new MenuItem("MURAMANA", "魔宗").SetValue(true));
            ...
            QuickSilverMenu.AddItem(new MenuItem("AnySnare", "束缚").SetValue(true));
            ...
            items.AddItem(
                new MenuItem("UseItemsMode", "物品使用模式").SetValue(
                    new StringList(new[] {"不使用", "混合模式", "连招模式", "都使用"}, 2)));
            ...
            if (BaseType != CClass.GetType())
            {
                ...
                {
                    harass.AddItem(new MenuItem("HarassMana", "骚扰法力值控制").SetValue(new Slider(50, 100, 0)));
                ...
                {
                    laneclear.AddItem(
                        new MenuItem("LaneClearMana", "清线法力值控制").SetValue(new Slider(50, 100, 0)));
                ...
                */
                ...
                var drawing = new Menu("显示设置", "Drawings");
                if (CClass.DrawingMenu(drawing))
                {
                    drawing.AddItem(
                        new MenuItem("drawMinionLastHit", "可被击杀小兵").SetValue(new Circle(false,
                            System.Drawing.Color.GreenYellow)));
                    drawing.AddItem(
                        new MenuItem("drawMinionNearKill", "附近击杀小兵").SetValue(new Circle(false,
                            System.Drawing.Color.Gray)));
                    drawing.AddItem(new MenuItem("drawJunglePosition", "无伤打野位置").SetValue(true));
