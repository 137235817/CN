...
namespace Marksman
{
    internal class Tristana : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("UseRM" + Id, "使用R击杀").SetValue(true));
            config.AddItem(new MenuItem("UseRMG" + Id, "使用R防突进").SetValue(true));
            config.AddItem(new MenuItem("UseRMI" + Id, "使用R打断技能").SetValue(true));
            return true;
        }
