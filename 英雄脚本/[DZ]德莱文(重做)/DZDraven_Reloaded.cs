...
namespace DZDraven_Reloaded
{
    class DZDraven_Reloaded
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Menu = new Menu("[DZ]德莱文", "DravenReloaded", true);
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("QManaC", "Q法力值控制").SetValue(new Slider(30, 1)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("WManaC", "W法力值控制").SetValue(new Slider(25, 1)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("EManaC", "E法力值控制").SetValue(new Slider(20, 1)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("RManaC", "R法力值控制").SetValue(new Slider(5, 1)));
            ...
            Menu.SubMenu("Harrass").AddItem(new MenuItem("QManaH", "Q法力值控制").SetValue(new Slider(30, 1)));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("WManaH", "Q法力值控制").SetValue(new Slider(25, 1)));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("EManaH", "Q法力值控制").SetValue(new Slider(20, 1)));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("RManaH", "Q法力值控制").SetValue(new Slider(5, 1)));
            ...
            Menu.SubMenu("Farm").AddItem(new MenuItem("UseELC", "使用Q").SetValue(true));
            Menu.SubMenu("Farm").AddItem(new MenuItem("QManaLC", "Q法力值控制").SetValue(new Slider(25, 1)));
            Menu.SubMenu("Farm").AddItem(new MenuItem("EManaLC", "Q法力值控制").SetValue(new Slider(25, 1)));
            Menu.SubMenu("Farm").AddItem(new MenuItem("CAF", "自动接斧头").SetValue(true));

            Menu.AddSubMenu(new Menu("接斧头设置", "Axes"));
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeC", "连招最大接斧头数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeH", "骚扰最大接斧头数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeF", "补兵最大接斧头数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes")
                .AddItem(new MenuItem("CatchRadius", "接斧头半径").SetValue(new Slider(600, 200, 1000)));
            Menu.SubMenu("Axes")
                .AddItem(new MenuItem("SafeZone", "接斧头区域").SetValue(new Slider(125, 0, 325)));
            Menu.AddSubMenu(new Menu("其他", "Misc"));
            Menu.SubMenu("Misc").AddItem(new MenuItem("ManualR", "手动大招").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Menu.SubMenu("Misc").AddItem(new MenuItem("Packets", "封包释放").SetValue(true));
            Menu.SubMenu("Misc").AddItem(new MenuItem("AntiGP", "E防突进")).SetValue(true);
            Menu.SubMenu("Misc").AddItem(new MenuItem("Interrupt", "E打断技能").SetValue(true));
            Menu.SubMenu("Misc").AddItem(new MenuItem("WCatch", "使用W接斧头").SetValue(true));
            Menu.SubMenu("Misc").AddItem(new MenuItem("WCatchCombo", "连招中使用").SetValue(false));
            ...
            Menu.SubMenu("Items").AddItem(new MenuItem("BilgeC", "连招使用比尔吉沃特弯刀").SetValue(true));
            Menu.SubMenu("Items").AddItem(new MenuItem("BilgeH", "骚扰使用比尔吉沃特弯刀").SetValue(false));
            Menu.SubMenu("Items").AddItem(new MenuItem("OwnHPercBotrk", "自身血量低于%使用破败").SetValue(new Slider(50, 1, 100)));
            ...
            Menu.AddSubMenu(new Menu("水银解除减益效果", "QSST"));
            Cleanser.CreateTypeQSSMenu();

            Menu.AddSubMenu(new Menu("水银解控", "QSSSpell"));
            ...
            Game.PrintChat("<font color='#FF0000'>[DZ]Draven</font> 鍔犺浇鎴愬姛 Vee姹夊寲!");
