...
namespace Xerath
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("R").AddItem(new MenuItem("EnableRUsage", "自动释放").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("rMode", "模式:").SetValue(new StringList(new[] { "正常", "自定义延迟", "优先"})));
            Config.SubMenu("R").AddItem(new MenuItem("rModeKey", "手动").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.SubMenu("R").AddSubMenu(new Menu("Custom delays", "自定义延迟"));
            for (int i = 1; i <= 3; i++)
                Config.SubMenu("R").SubMenu("Custom delays").AddItem(new MenuItem("Delay"+i, "延迟"+i).SetValue(new Slider(0, 1500, 0)));
            Config.SubMenu("R").AddItem(new MenuItem("PingRKillable", "可击杀提示 (本地ping)").SetValue(true));
            ...
            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterR", "显示三炮后的伤害").SetValue(true);
            ...
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("WRange", "W范围").SetValue(new Circle(true, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("ERange", "E范围").SetValue(new Circle(false, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("RRange", "R范围").SetValue(new Circle(false, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("RRangeM", "R范围(小地图)").SetValue(new Circle(false,
            ...
            Game.PrintChat(ChampionName + " 鍔犺浇鎴愬姛 Vee姹夊寲");
