...
namespace Cassiopeia
{
    class Program
    {
        ...
        static Menu menu = new Menu("[Huntera]魔蛇之拥", "Cassiopeia", true);
		...
		Game.PrintChat("[Huntera]榄旇泧涔嬫嫢-鍔犺浇鎴愬姛   Vee姹夊寲");
            ...
            menu.SubMenu("LineClear").AddItem(new MenuItem("onlylasthitE", "补刀使用E").SetValue(true));
            ...
            menu.SubMenu("Ultimate").SubMenu("AntiGapcloser").AddItem(new MenuItem("gapcloserR", "使用R防突进").SetValue(true));
            menu.SubMenu("Ultimate").SubMenu("Interruptspells").AddItem(new MenuItem("interruptR", "使用R打断法术").SetValue(true));
            ...
            menu.SubMenu("Misc").AddItem(new MenuItem("castedalay", "施放E延迟").SetValue(new Slider(0, 0, 2000)));
            menu.SubMenu("Misc").AddItem(new MenuItem("castWPoisoned", "使用W（当目标未中毒）").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("focusSelectedTarget", "优先攻击选择目标").SetValue(false));
            ...
            menu.AddSubMenu(new Menu("范围显示", "Drawings"));
            ...
            menu.AddSubMenu(new Menu("Vee汉化", "Vee"));						
