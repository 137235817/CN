...
namespace FedDrMundo
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("[Fed]蒙多", ChampionName, true);
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("UseItensCombo", "使用物品").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("ComboActive", "连招").SetValue(new KeyBind(32, KeyBindType.Press)));            

            Config.AddSubMenu(new Menu("骚扰", "Harass"));            
            Config.SubMenu("Harass").AddItem(new MenuItem("LifeHarass", "生命值低于%不使用骚扰").SetValue(new Slider(30, 100, 0)));
            ...
            Config.SubMenu("JungleFarm").AddItem(new MenuItem("AutoSmite", "自动惩戒").SetValue<KeyBind>(new KeyBind('J', KeyBindType.Toggle)));
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("laugh", "大笑（嘲讽）").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("KS", "Q击杀").SetValue(false));
            Config.SubMenu("Misc").AddItem(new MenuItem("AutoI", "自动引燃").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("RangeQ", "Q范围").SetValue(new Slider(980, 1000, 0)));
            Config.SubMenu("Misc").AddItem(new MenuItem("lifesave", "大招回复生命值").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("percenthp", "生命值低于%使用大招").SetValue(new Slider(30, 100, 0)));

            var dmgAfterComboItem = new MenuItem("DamageAfterCombo", "显示一套技能的伤害").SetValue(true);
            ...
            TargetedItems = new Menu("单体物品", "menuTargetItems");
            menuUseItems.AddSubMenu(TargetedItems);
            TargetedItems.AddItem(new MenuItem("item3153", "破败").SetValue(true));
            TargetedItems.AddItem(new MenuItem("item3143", "兰盾").SetValue(true));
            TargetedItems.AddItem(new MenuItem("item3144", "比尔吉沃特弯刀").SetValue(true));
            TargetedItems.AddItem(new MenuItem("item3146", "海克斯的科技枪").SetValue(true));
            TargetedItems.AddItem(new MenuItem("item3184", "冰霜战锤").SetValue(true));
            
            NoTargetedItems = new Menu("范围物品", "menuNonTargetedItems");
            ...
            Game.PrintChat("<font color=\"#00BFFF\">[Fed]" + ChampionName + " -</font> <font color=\"#FFFFFF\">鍔犺浇鎴愬姛 Vee姹夊寲</font>");
