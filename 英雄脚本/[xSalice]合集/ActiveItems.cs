...
namespace xSaliceReligionAIO
{
    class ActiveItems
    {
        ...
        public static void AddToMenu(Menu theMenu)
        {
            ...
            var offensiveItem = new Menu("攻击物品", "Offensive Items");
            {
            ...
            {
                var ignite = new Menu("引燃", "Ignite");
                {
                    ignite.AddItem(new MenuItem("ignite", "使用引燃", true).SetValue(true));
                    ignite.AddItem(
                        new MenuItem("igniteMode", "引燃模式", true).SetValue(new StringList(new[] {"连招", "击杀"})));
        ...
        private static void AddOffensiveItem(Menu subMenu, ActiveItems item)
        {
            var active = new Menu(item.ActiveName, item.ActiveName);
            {
                ...
                active.AddItem(new MenuItem(item.ActiveName + "dmgCalc", "添加伤害计算", true).SetValue(true));
                active.AddItem(new MenuItem(item.ActiveName + "killAble", "敌人可击杀时使用", true).SetValue(false));
                active.AddItem(new MenuItem(item.ActiveName + "always", "总是", true).SetValue(item.Mode == 1 || item.Mode == 2));
                active.AddItem(new MenuItem(item.ActiveName + "myHP", "自己生命值低于%使用", true).SetValue(new Slider(25)));
                active.AddItem(new MenuItem(item.ActiveName + "enemyHP", "目标生命值低于%使用", true).SetValue(new Slider(50)));
