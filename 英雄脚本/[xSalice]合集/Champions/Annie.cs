    ...
    class Annie : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("按键", "Key");
            {
                ...
                key.AddItem(new MenuItem("LastHitQ", "使用Q补刀!", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("LastHitQToggle", "使用Q补刀(自动)", true).SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("FlashKILL", "闪现击杀", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("ChargeStun", "攒晕(自动)!", true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Toggle)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能设置", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Stun_LastHit", "使用Q补刀攒晕", true).SetValue(false));
                    spellMenu.AddSubMenu(qMenu);
                }

                var wMenu = new Menu("W设置", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("useW_enemyCount", "可晕敌人>X自动W", true).SetValue(new Slider(3, 1, 5)));
                    wMenu.AddItem(new MenuItem("W_charge", "使用W攒晕", true).SetValue(true));
                    spellMenu.AddSubMenu(wMenu);
                }

                var eMenu = new Menu("E设置", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Charge", "使用E攒晕", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Block", "使用E阻挡攻击", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Stun", "优先使用E攒晕", true).SetValue(true));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R设置", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("useR_enemyCount", "可晕敌人>X自动R, 6 = 关闭", true).SetValue(new Slider(3, 1, 6)));
                    rMenu.AddItem(new MenuItem("R_To_KS", "使用R击杀", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("R_OverKill", "击杀检查", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("R_Mode", "泰伯熊模式", true).SetValue(new StringList(new[] { "跟随 目标", "跟随 自己" })));
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("R_Killable", "使用R能击杀时", true).SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Toggle)));
                combo.AddItem(new MenuItem("useR_enemyCount_Combo", "可晕敌人>X使用R", true).SetValue(new Slider(3, 1, 5)));
                combo.AddItem(new MenuItem("FlashStun", "使用闪现->R->晕人击杀", true).SetValue(true));
                //add to menu
            ...
            var farm = new Menu("清线", "LaneClear");
            {
                ...
                farm.AddItem(new MenuItem("LaneClear_useW_minHit", "使用W(命中数)", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            {
                misc.AddItem(new MenuItem("Interrupt", "使用眩晕打断法术", true).SetValue(true));
                misc.AddItem(new MenuItem("W_Gap", "使用眩晕防突进", true).SetValue(true));
                misc.AddItem(new MenuItem("smartKS", "使用智能击杀系统", true).SetValue(true));
                misc.AddItem(new MenuItem("chargeMana", "攒晕丨MP >=", true).SetValue(new Slider(30)));
                misc.AddItem(new MenuItem("chargeInFountain", "泉水攒晕", true).SetValue(true));
                misc.AddItem(new MenuItem("disableAA", "技能CD好之后不平A", true).SetValue(false));
                menu.AddSubMenu(misc);
            }

            var drawMenu = new Menu("显示", "Drawing");
            {
                drawMenu.AddItem(new MenuItem("Draw_Disabled", "禁止所有显示", true).SetValue(false));
                drawMenu.AddItem(new MenuItem("Draw_Q", "显示Q范围", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_W", "显示W范围", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_R", "显示R范围", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_R2", "显示闪现-R范围", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("FlashStunTarget", "显示闪现击杀状态", true).SetValue(true));

                MenuItem drawComboDamageMenu = new MenuItem("Draw_ComboDamage", "显示连招伤害", true).SetValue(true);
                MenuItem drawFill = new MenuItem("Draw_Fill", "显示全部连招伤害", true).SetValue(new Circle(true, Color.FromArgb(90, 255, 169, 4)));
