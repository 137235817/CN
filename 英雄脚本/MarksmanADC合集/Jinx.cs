...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("SwapQ" + Id, "自动切换Q").SetValue(false));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapDistance" + Id, "根据距离切换Q").SetValue(true));
            config.AddItem(new MenuItem("SwapAOE" + Id, "切换Q打AOE伤害").SetValue(false));
            ...
            config.AddItem(
                new MenuItem("CastR" + Id, "手动释放R(2000)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("ROverKill" + Id, "检查R击杀伤害").SetValue(true));
            config.AddItem(new MenuItem("MinRRange" + Id, "最小R范围").SetValue(new Slider(300, 0, 1500)));
            config.AddItem(new MenuItem("MaxRRange" + Id, "最大R范围").SetValue(new Slider(1700, 0, 4000)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "显示Q范围").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 0))));
            config.AddItem(
                new MenuItem("DrawW" + Id, "显示W范围").SetValue(new Circle(false, Color.CornflowerBlue)));
            return true;
        }
