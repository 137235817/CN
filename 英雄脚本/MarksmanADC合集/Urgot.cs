...
namespace Marksman
{
    internal class Urgot : Champion
    {
        ...
        public override bool ComboMenu(Menu config)
        {
            ...
            config.AddSubMenu(new Menu("大招选项1", "UltOpt1"));

            config.SubMenu("UltOpt1")
                .AddItem(
                    new MenuItem("UltOp1" + Id, "把敌人传送到到友方炮塔下").SetValue(new KeyBind("T".ToCharArray()[0],
                        KeyBindType.Press)));

            config.AddSubMenu(new Menu("大招选项2", "UltOpt2"));
            config.SubMenu("UltOpt2")
                .AddItem(
                    new MenuItem("UltOp2" + Id, "把敌人传送到队友身边").SetValue(new KeyBind("G".ToCharArray()[0],
                        KeyBindType.Press)));
            config.SubMenu("UltOpt2")
                .AddItem(new MenuItem("UltOp2Count" + Id, "传送时|最小友方数").SetValue(new Slider(1, 1, 5)));


            config.AddSubMenu(new Menu("不对XX使用大招", "DontUlt"));
        ...
        {
            config.AddItem(
                new MenuItem("DrawQ" + Id, "Q范围").SetValue(new Circle(true, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawE" + Id, "E范围").SetValue(new Circle(false, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawR" + Id, "R范围").SetValue(new Circle(false, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawQEx" + Id, "腐蚀控制").SetValue(new Circle(true, Color.LightGray)));
        ...
        {
            config.AddItem(new MenuItem("UseQL" + Id, "使用Q").SetValue(true));
            return true;
        }
