...
namespace Marksman
{
    internal class Ashe : Champion
    {
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseWTH" + Id, "使用W (自动)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawW" + Id, "W范围").SetValue(new Circle(true, Color.CornflowerBlue)));
            config.AddItem(
                new MenuItem("DrawE" + Id, "E范围").SetValue(new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("RInterruptable" + Id, "自动大招打断技能").SetValue(true));
            config.AddItem(new MenuItem("EFlash" + Id, "对敌人闪现后位置使用E").SetValue(true));
            config.AddItem(new MenuItem("RManualCast" + Id, "手动大招"))
