...
namespace Marksman
{
    internal class Vayne : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("UseET" + Id, "使用E(自动)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseEInterrupt" + Id, "使用E打断").SetValue(true));
            config.AddItem(
                new MenuItem("PushDistance" + Id, "E推开距离").SetValue(new Slider(425, 475, 300)));
            config.AddItem(new MenuItem("CompleteSilverBuff" + Id, "Q激活被动").SetValue(true));
            return true;
        }
        ...
        {
            config.AddItem(new MenuItem("UseQL" + Id, "使Q").SetValue(true));
            return true;
        }
