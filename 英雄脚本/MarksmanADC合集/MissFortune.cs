...
namespace Marksman
{
    internal class MissFortune : Champion
    {
        ...
        public override bool ComboMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("UseQMC", "使用Q(小兵弹到英雄)").SetValue(true));
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseQTH" + Id, "使用Q(自动)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            config.AddItem(
                new MenuItem("UseETH" + Id, "使用E(自动)").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Toggle)));
        ...
        {
            config.AddItem(
                new MenuItem("DrawQ" + Id, "Q范围").SetValue(
                    new Circle(true, System.Drawing.Color.FromArgb(100, 255, 0, 255))));

            config.AddItem(
                new MenuItem("DrawE" + Id, "E范围").SetValue(
