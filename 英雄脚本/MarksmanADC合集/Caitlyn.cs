...
namespace Marksman
{
    internal class Caitlyn : Champion // Base done by xQx, Drawings and improvements added by Dibes.
    {
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawE" + Id, "E范围").SetValue(new Circle(false, Color.FromArgb(100, 255, 255, 255))));
            config.AddItem(
                new MenuItem("DrawR" + Id, "R范围").SetValue(new Circle(false, Color.FromArgb(100, 255, 255, 255))));
            config.AddItem(
                new MenuItem("DrawUlt" + Id, "大招提示").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 255))));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("UltHelp" + Id, "对目标使用大招").SetValue(new KeyBind("R".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("UseEQC" + Id, "使用E-Q连招").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("Dash" + Id, "向鼠标方向使用E").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }

        public override bool ExtrasMenu(Menu config)
        {
            config.AddItem(new MenuItem("AutoQI" + Id, "自动Q (眩晕/束缚/嘲讽/减速)").SetValue(true));
            config.AddItem(new MenuItem("AutoWI" + Id, "自动W (眩晕/束缚/嘲讽)").SetValue(true));
            return true;
        }
