...
namespace Marksman
{
    ...
    internal class Draven : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("UseQC", "使用Q").SetValue(true));
            config.AddItem(new MenuItem("UseWC", "使用W").SetValue(true));
            config.AddItem(new MenuItem("UseEC", "使用E").SetValue(true));
            config.AddItem(new MenuItem("UseRC", "使用R").SetValue(true));
            return true;
        }

        public override bool HarassMenu(Menu config)
        {
            config.AddItem(new MenuItem("UseQH", "使用Q").SetValue(true));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawE", "E范围").SetValue(new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawR", "R范围(2000)").SetValue(
        ...
        public override bool MiscMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("EGapCloser", "自动E被突进").SetValue(true));
            config.AddItem(new MenuItem("EInterruptable", "自动E打断技能").SetValue(true));
            config.AddItem(new MenuItem("RManualCast", "手动R(2000)"))
                .SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press));
            config.AddItem(new MenuItem("Epeel", "使用E减速敌人逃走").SetValue(true));
