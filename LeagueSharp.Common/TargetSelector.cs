...
namespace LeagueSharp.Common
{
    public class TargetSelector
    {
        ...
        {
            _configMenu = config;
            config.AddItem(new MenuItem("FocusSelected", "优先攻击选中的目标").SetShared().SetValue(true));
            config.AddItem(
                new MenuItem("ForceFocusSelected", "只攻击选中的目标").SetShared().SetValue(false));
            config.AddItem(
                new MenuItem("SelTColor", "选中目标颜色").SetShared().SetValue(new Circle(true, Color.Red)));
            config.AddItem(new MenuItem("Sep", "").SetShared());
            var autoPriorityItem = new MenuItem("AutoPriority", "自动排列优先攻击目标").SetShared().SetValue(false);
