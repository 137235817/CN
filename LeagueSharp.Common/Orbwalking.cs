    ...
    public static class Orbwalking
    {
        ...
        public class Orbwalker
        {
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                drawings.AddItem(
                    new MenuItem("AACircle", "平A范围").SetShared()
                        .SetValue(new Circle(true, Color.FromArgb(255, 255, 0, 255))));
                drawings.AddItem(
                    new MenuItem("AACircle2", "敌人平A范围").SetShared()
                        .SetValue(new Circle(false, Color.FromArgb(255, 255, 0, 255))));
                drawings.AddItem(
                    new MenuItem("HoldZone", "待命区域范围").SetShared()
                        .SetValue(new Circle(false, Color.FromArgb(255, 255, 0, 255))));
                _config.AddSubMenu(drawings);

                /* Misc options */
                var misc = new Menu("其它设置", "Misc");
                misc.AddItem(
                    new MenuItem("HoldPosRadius", "待命区域半径").SetShared().SetValue(new Slider(0, 0, 150)));
                misc.AddItem(new MenuItem("PriorizeFarm", "优先补刀后骚扰").SetShared().SetValue(true));
                _config.AddSubMenu(misc);


                /* Delay sliders */
                _config.AddItem(
                    new MenuItem("ExtraWindup", "平A延迟").SetShared().SetValue(new Slider(80, 0, 200)));
