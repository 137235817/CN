...
namespace KaiHelper.Tracker
{
    ...
    public class GankDetector
    {
        ...
        public GankDetector(Menu config)
        {
            ...
            _menuGank.AddItem(new MenuItem("InvisibleTime", "消失计时").SetValue(new Slider(5, 1, 10)));
            _menuGank.AddItem(new MenuItem("VisibleTime", "出现计时").SetValue(new Slider(3, 1, 5)));
            ...
            _menuGank.AddItem(new MenuItem("GankType", "类型").SetValue(new StringList(new []{"线","圈","同时"})));
            _menuGank.AddItem(new MenuItem("Ping", "聊天框提示").SetValue(true));
            _menuGank.AddItem(new MenuItem("GankActive", "开启").SetValue(true));
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            foreach (var enemy in _enemies)
            {
                ...
                if (!_enemies[hero].Pinged)
                {
                    ...
                    Game.PrintChat("<font color = \"#FF0000\">閫楁瘮鏉ユ姄浣犱簡: </font>" + hero.ChampionName);
