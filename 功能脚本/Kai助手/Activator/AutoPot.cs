...
namespace KaiHelper.Activator
{
    internal class AutoPot
    {
        ...
        {
            _menu = menu.AddSubMenu(new Menu("吃药 管理", "PotionManager"));
            _menu.AddItem(new MenuItem("HPTrigger", "血瓶|血量低于%").SetValue(new Slider(30)));
            _menu.AddItem(new MenuItem("HealthPotion", "开启自动吃红药").SetValue(true));
            _menu.AddItem(new MenuItem("MPTrigger", "蓝瓶|蓝量低于%").SetValue(new Slider(30)));
            _menu.AddItem(new MenuItem("ManaPotion", "开启自动吃蓝药").SetValue(true));
            MenuItem autoarrangeMenu = _menu.AddItem(new MenuItem("AutoArrange", "自动整理").SetValue(false));
