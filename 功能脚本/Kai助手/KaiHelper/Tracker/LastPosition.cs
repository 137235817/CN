...
namespace KaiHelper.Tracker
{
    internal class LastPosition
    {
        ...
        {
            Menu = timer.AddSubMenu(new Menu("最后消失地点(开启后F5重载)", "Last Position"));
            Menu.AddItem(new MenuItem("Scale", "图像尺寸").SetValue(new Slider(20, 1, 50)));
            Menu.AddItem(new MenuItem("Opacity", "不透明度").SetValue(new Slider(70)));
            Menu.AddItem(new MenuItem("TextSize", "文字大小").SetValue(new Slider(15, 1)));
            Menu.AddItem(new MenuItem("ALP", "开启").SetValue(true));
            CustomEvents.Game.OnGameLoad += Game_OnGameLoad;
            Obj_AI_Base.OnTeleport += ObjAiBaseOnOnTeleport;
            Game.OnGameUpdate += Game_OnGameUpdate;
            Game.PrintChat("敌方消失地点提示-加载成功!");
        }
