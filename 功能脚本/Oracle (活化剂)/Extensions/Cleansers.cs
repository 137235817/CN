...
namespace Oracle.Extensions
{
    internal static class Cleansers
    {
        ...
        public static void Initialize(Menu root)
        {
            ...
            _mainMenu = new Menu("自动清除减益", "cmenu");
            _menuConfig = new Menu("清除减益配置", "cconfig");
        ...
        private static void CreateMenuItem(string displayname, string name, int ccvalue)
        {
            ...
            menuName.AddItem(new MenuItem(name + "Count", "清除减益（数量）"));//.SetValue(new Slider(ccvalue, 1, 5));
            menuName.AddItem(new MenuItem(name + "Duration", "清除减益（持续时间）"));//.SetValue(new Slider(2, 1, 5));
